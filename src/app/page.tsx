...
export default function KrsSimulatorGacor() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 font-sans flex flex-col xl:flex-row gap-8 relative overflow-hidden">
      ...
      >
        <div className="bg-gray-900 border border-slate-700 shadow-2xl rounded-2xl p-4 flex gap-4 items-start">
          ...
          {matkulSemesterIni.map((matkul) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    className={`mt-1 flex items-center justify-center w-6 h-6 rounded-lg border-2 transition-colors ${diambil ? "bg-pink-500 border-pink-500" : "bg-white border-slate-300"}`}
                  >
                    {diambil && (
                      <svg
                  ...
                  <div className="flex-1">
                    <p
                      className={`font-bold text-[14px] leading-snug tracking-tight ${diambil ? "text-indigo-950" : "text-slate-700"}`}
                    >
                      {matkul.nama}
                    </p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-[10px] px-2 py-0.5 rounded-md bg-slate-100 text-slate-500 font-bold tracking-wider">
                        {matkul.kode}
                      </span>
                              ...
                              {paralelUnik.map((pNo: number) => {
                                ...
                                return (
                                  <option key={pNo} value={pNo}>
                                    {tipe}{pNo} | {info?.hari}, {info?.jam_mulai}-
                                    {info?.jam_selesai}
                                  </option>
                                );
                              })}
      ...
      <div className="flex-1 flex flex-col gap-6 relative z-10">
        ...
        <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden">
          {/* ================= TAMPILAN MOBILE (AGENDA VERTIKAL) ================= */}
              ...
              <div className="flex flex-col gap-5">
                {days.map((day) => {
                  const jadwalHariIni = jadwalAktif
                    ...
                    .sort(
                      (a, b) => parseTime(a.jam_mulai) - parseTime(b.jam_mulai),
                    );
                  ...
                  return (
                    <div key={day} className="flex flex-col gap-3">
                      ...
                      <div className="flex items-center">
                        <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] font-black uppercase tracking-widest border border-slate-200 shadow-sm">
                          {day}
                        </span>
                      </div>

                      {/* Garis Timeline & Kartu Kelas */}
                      <div className="flex flex-col gap-3 border-l-2 border-slate-100 ml-4 pl-4 py-1">
                        {jadwalHariIni.map((kelas, idx) => {
                          const styleKategori =
                            theme[kelas.tipe as TipeKegiatan];
                          return (
                            <div
                              key={idx}
                              className={`relative rounded-xl p-3 shadow-sm ${styleKategori}`}
                            >
          ...
          {/* ================= TAMPILAN DESKTOP (GRID KALENDER) ================= */}
              ...
              {Object.keys(pilihan).length === 0 ? (
                <div className="p-8 text-center border border-dashed border-slate-200 rounded-2xl">
                  ...
                  <p className="text-xs text-slate-300 mt-1">
                    Silakan pilih mata kuliah di panel atas.
                  </p>
                </div>
              ) : (
                  ...
                  {Object.keys(pilihan).length === 0 ? (
                    <tr>
                      <td colSpan={3} className="p-12 text-center">
                        ...
                        <p className="text-xs text-slate-300 mt-1">
                          Silakan pilih mata kuliah dengan jadwal yang tidak
                          bentrok.
                        </p>
                      </td>
                    </tr>
